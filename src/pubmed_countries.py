#!/usr/bin/env python3
"""This module contains synonyms for countries used in PubMed."""

# -*- coding: utf-8 -*-
Afghanistan = ['Afghanistan']
Albania = ['Albania']
Algeria = ['Algeria']
Andorra = ['Andorra']
Angola = ['Angola']
Antigua = ['Antigua and Barbuda']
Argentina = ['Argentina', 'Facultad de Ciencias Médicas']
Armenia = ['Armenia']
Australia = ['Australia', 'Sydney', 'CSANZ', 'Queensland', 'Auckland',
             '4032', 'Melbourne', 'VIC', 'AU', 'Jen-Teh',
             'mcardona@bond.edu.au']
Austria = ['Austria', 'Österreich']
Azerbaijan = ['Azerbaijan']
Bahamas = ['Bahamas']
Bahrain = ['Bahrain', 'BHR']
Bangladesh = ['Bangladesh']
Barbados = ['Barbados']
Belarus = ['Belarus']
Belgium = ['Belgium', 'Belgio', 'Belgique']
Benin = ['Benin']
Bhutan = ['Bhutan']
Bolivia = ['Bolivia']
Bosnia = ['Bosnia and Herzegovina', 'Bosnia', 'Herzegovina']
Brazil = ['Brazil', 'Brasil', 'Brasil.', 'Reprodução Humana Monteleone',
          'São Paulo', 'BR', 'Rio de Janeiro', 'Brazi']
Brunei = ['Brunei', 'Brunei Darussalam', 'Cirurgiões', 'Brasileiro',
          'swaldo Cruz']
Bulgaria = ['Bulgaria']
Burkina_Faso = ['Burkina Faso']
Cabo_Verde = ['Cabo Verde']
Canada = ['Canada', 'CMAJ', 'Ontario', 'British Columbia', 'H3G 1A4',
          'Hamilton', 'Ont', 'Ottawa', 'Toronto', 'V6T 1Z3', 'Manitoba',
          'V6H3V4', 'BC']
Cambodia = ['Cambodia']
Cameroon = ['Cameroon']
Central_Africa = ['Central African Republic']
Chad = ['Chad']
Chile = ['Chile']
China = ['China', 'Anhui', 'Beijing', 'Chongqing', 'Fujian', 'Gansu',
         'Guangdong', 'Guangxi', 'Guizhou', 'Hainan', 'Hebei', 'Hong Kong',
         'Hubei', 'Hunan', 'Inner Mongolia', 'Jiangsu', 'Jiangxi', 'Jilin',
         'Liaoning', 'Macau', 'Ningxia', 'Qinghai', 'Shaanxi', 'Shandong',
         'Shanxi', 'Sichuan', 'Tianjin', 'Tibet', 'Xinjiang', 'Zhejiang',
         'Soochow', 'Nanjing', 'Wuhan', '610072', 'Binwu Ying', '430071',
         'Macao', 'hina', 'CN', 'Huazhong', 'hina.', 'Zhengzhou', 'Chengdu',
         '430022', '710061', '200032', 'Chin', 'uoshenshan Hospital',
         'ccmu.edu.cn', 'Sun-Yat', 'HongKong', 'Changsha',
         'Central South University', 'Xiamen', '430056',
         'Three Gorges University', 'Kong Red Cross Blood Transfusion Service',
         '510000', 'Harbin', 'Taiyuan', 'penghong66@csu.edu.cn',
         'fmmuzhr@fmmu.edu.cn', '100871']
Colombia = ['Colombia']
Congo = ['Congo', 'Congo (Brazzaville)', 'Congo (Kinshasa)', 'Brazzaville',
         'Kinshasa']
Costa_Rica = ['Costa Rica']
Cote_d_Ivoire = ['Cote d Ivoire']
Croatia = ['Croatia', 'Turke', 'Zagreb', 'Osijek']
# Turke - possible issue with Turkey
Cuba = ['Cuba']
Cyprus = ['Cyprus']
Denmark = ['Denmark', 'lefja@rm.dk']
Djibouti = ['Djibouti']
Dominican_Republic = ['Dominican Republic', 'DOM']
Ecuador = ['Ecuador']
Egypt = ['Egypt', 'Suez Canal']
El_Salvador = ['El Salvador']
Equatorial_Guinea = ['Equatorial Guinea']
Eritrea = ['Eritrea']
Estonia = ['Estonia', 'EE']
Eswatini = ['Eswatini']
Ethiopia = ['Ethiopia']
Fiji = ['Fiji']
Finland = ['Finland', 'FI']
Faroe = ['Faroe Islands']
France = ['France', 'French Guiana', 'French Polynesia', 'Guadeloupe',
          'Mayotte', 'New Caledonia', 'Reunion', 'Saint Barthelemy',
          'St Martin', 'Martinique', 'Saint Pierre', 'Miquelon', '.fr',
          'Lille', 'FR', 'Paris', 'Lyon', 'Saint-Andre Hospital']
Gabon = ['Gabon']
Gambia = ['Gambia']
Georgia = ['Georgia']
Germany = ['Germany', 'Deutschland', 'Berlin', 'Heidelberg', 'Dresden',
           'Homburg', 'Homburg/Saar', 'Bielefeld', 'Ameos Klinikum', 'Leipzig',
           'Freiburg', 'Universitätsklinikum',
           'Klinik und Poliklinik für Hals-. Nasen- und Ohrenheilkunde',
           'Plastische und Ästhetische Chirurgie',
           'Köln', 'Mainz', 'Munich', '.de', 'Erkelenz', 'Münster',
           'Angiologie und Pneumologie']
Ghana = ['Ghana']
Greece = ['Greece', 'University Hospital of Patras', 'Patras.', 'GRC']
Guatemala = ['Guatemala']
Guinea = ['Guinea']
Guyana = ['Guyana']
Haiti = ['Haiti']
Honduras = ['Honduras']
Hungary = ['Hungary', 'Budapest', 'Szeged']
Iceland = ['Iceland']
India = ['India', '400014', 'Delhi', 'Maharashtra', 'Gandhi', 'Nagpur',
         'Hyderabad', 'Haryana', 'Uttarakhand', 'Manipur', 'Lucknow', '143117',
         'Trivandrum', 'Kolkata', 'Jodhpur', 'Indi', 'Bagalkot']
Indonesia = ['Indonesia']
Iran = ['Iran', 'Tehran', 'ac.ir', 'IR']
Iraq = ['Iraq']
Ireland = ['Ireland', 'IRL', 'Dublin', 'Limerick', 'Wicklow']
Israel = ['Israel', 'Gertner Institute']
Italy = ['Italy', 'Holy See', 'See of Rome', 'Vatican', 'Umbria', 'talian',
         'Pisa', 'Milan', 'Acta Biomedica', 'IRCCS', 'Brescia', 'Lodi', 'Rome',
         'Rome.', 'Ital', 'Roma', 'Policlinico San Matteo', 'Italia',
         'Genova', 'Bologna', 'Casalecchio di Reno', 'Sapienza', 'Verona',
         'Padova', 'Fermo', 'Università', '.it', 'Negev', 'unibs.it',
         'University del Piemonte Orientale', 'ITA', 'Trento', 'Sicily',
         'taly', 'Policlinico di Bari', 'Torino', 'Piacenza', 'Firenze',
         'Siena', 'Pavia']
Jamaica = ['Jamaica']
Japan = ['Japan', 'Japa', 'Yokohama', 'Tohoku', 'Keio',
         'Furano Kyokai Hospital', 'Showa University']
Jordan = ['Jordan', 'JO']
Kazakhstan = ['Kazakhstan']
Kenya = ['Kenya']
South_Korea = ['Korea, South', 'South Korea', 'Korea', 'KOREA', 'KOR',
               'Coreia do Sul']
Kuwait = ['Kuwait']
Kyrgyzstan = ['Kyrgyzstan']
Latvia = ['Latvia']
Lebanon = ['Lebanon', 'LBN']
Liberia = ['Liberia']
Liechtenstein = ['Liechtenstein']
Lithuania = ['Lithuania']
Luxembourg = ['Luxembourg', '1145']
Madagascar = ['Madagascar']
Malaysia = ['Malaysia', 'Kelantan']
Maldives = ['Maldives']
Malta = ['Malta']
Mauritania = ['Mauritania']
Mauritius = ['Mauritius']
Mexico = ['Mexico', 'México']
Moldova = ['Moldova']
Monaco = ['Monaco']
Mongolia = ['Mongolia']
Montenegro = ['Montenegro']
Morocco = ['Morocco', 'Maroc']
Namibia = ['Namibia']
Nepal = ['Nepal']
Netherlands = ['Netherlands', 'Aruba', 'Curacao', 'Sint Maarten',
               'Bonaire, Sint Eustatius and Saba', 'etherlands', 'NL',
               'Nijmegen', 'Hilversum', 'Breda', 'Uden', 'Tilburg',
               'Amersfoort', 'filosofie en geschiedenis', 'Helmond', 'Leiden',
               'Buitenhof', 'Amsterdam']
New_Zealand = ['New Zealand', 'Otago', 'Zealand',
               'Southern District Health Board']
Nicaragua = ['Nicaragua']
Niger = ['Niger']
Nigeria = ['Nigeria', 'igeria']
Macedonia = ['Macedonia']
Norway = ['Norway', 'Alta', 'Bergen', 'St Olavs University Hospital']
Oman = ['Oman']
Pakistan = ['Pakistan', 'Karachi', 'PAK', 'Faisalabad', 'Islamabad']
Palestine = ['Palestinian']
Panama = ['Panama']
Papua_New_Guinea = ['Papua New Guinea']
Paraguay = ['Paraguay']
Peru = ['Peru', 'Perú']
Philippines = ['Philippines']
Poland = ['Poland', 'Warszawie', 'Warsaw', 'Katowicach', 'Śląski']
Portugal = ['Portugal', 'Algarve', 'São João']
Qatar = ['Qatar', 'Doha']
Romania = ['Romania', 'ROU']
Russia = ['Russia', 'Russian', 'Lazarski', 'Moscow']
Rwanda = ['Rwanda']
Scotland = ['Scotland', 'EH4 2XU']
Saint_Lucia = ['Saint Lucia']
Saint_Vincent = ['Saint Vincent and the Grenadines']
Samoa = ['Samoa']
San_Marino = ['San Marino']
Saudi_Arabia = ['Saudi Arabia', 'SAU']
Senegal = ['Senegal']
Serbia = ['Serbia']
Seychelles = ['Seychelles']
Singapore = ['Singapore', '308232', 'Singapor', 'SG', 'Changi']
Slovakia = ['Slovakia', 'SK']
Slovenia = ['Slovenia']
Somalia = ['Somalia']
South_Africa = ['South Africa', 'Cape Town', 'Cape Town.', 'ac.za', 'ZA']
Spain = ['Spain', 'Mallorca', 'España', 'Majadahonda', 'ES',
         'pilar.beato@salud-juntaex.es', 'Huelva']
Sri_Lanka = ['Sri Lanka']
Sudan = ['Sudan']
Suriname = ['Suriname']
Sweden = ['Sweden', '.se', 'Karolinska institutet', 'Capio ASiH/SPSV Nacka',
          'Skånes universitetssjukhus', 'Stockholm', 'Uppsala', 'Solna',
          'Swedish']
Switzerland = ['Switzerland', 'Lausanne', 'CH', 'État de Vaud', 'St. Gallen',
               'Genève', 'Zürich', 'Porrentruy']
Taiwan = ['Taiwan', 'Taiwan*', 'Taipei', 'ROC', 'Chang Gung']
Tanzania = ['Tanzania']
Thailand = ['Thailand', 'urapha University']
Togo = ['Togo']
Trinidad = ['Trinidad and Tobago', 'Trinidad', 'Tobago', 'TTO']
Tunisia = ['Tunisia']
Turkey = ['Turkey', 'Advisory Committee of the Ministry of Health of Turke',
          'İstanbul', 'TURKEY']
Uganda = ['Uganda']
Ukraine = ['Ukraine']
United_Arab_Emirates = ['United Arab Emirates', 'UAE', 'KSA', 'AE']
UK = ['UK', 'United Kingdom', 'Bermuda', 'Cayman Islands', 'Channel Islands',
      'Gibraltar', 'Isle of Man', 'Montserrat', 'Anguilla',
      'British Virgin Islands', 'Turks', 'Caicos Islands',
      'Turks and Caicos Islands', 'Falkland Islands', 'Malvinas', 'London',
      'Cambridge', 'Birmingham', 'England', 'Queen Elizabeth', 'NHS',
      'outhern Cross University', 'Southampton', 'Hertfordshire', 'Britain',
      'liverpool', 'ac.uk', 'U.K.', 'U.K', 'Oxford', 'kcl.ac.uk', 'Swansea',
      'GB', 'Chatham House', 'Montefiore', 'Reino Unido',
      'Queen Mary Hospital',
      'St Thomas', 'Worcester', 'Brighton', 'Salford', 'CF15 7QQ',
      'Centre for Arts Therapies', 'HA1 3UJ', '*.nhs.uk', 'Wales',
      'Marlies.Ostermann@gstt.nhs.uk', 'Wiltshire', 'SO16 6YD',
      'hartered Society of Physiotherapy', 'Ceres']
Uruguay = ['Uruguay', 'Uruguay.']
USA = ['USA', 'US', 'UNITED STATES', 'United States',
       'United States of America', 'America', 'Alabama', 'Alaska',
       'Arizona', 'Arkansas', 'California', 'Colorado', 'Connecticut',
       'Delaware', 'District of Columbia', 'Florida', 'Georgia', 'Hawaii',
       'Idaho', 'Illinois', 'Indiana', 'Iowa', 'Kansas', 'Kentucky',
       'Louisiana', 'Maine', 'Montana', 'Nebraska',
       'Nevada', 'New Hampshire', 'New Jersey', 'New Mexico', 'New York',
       'North Carolina', 'North Dakota', 'Ohio', 'Oklahoma', 'Oregon',
       'Maryland', 'Massachusetts', 'Michigan', 'Minnesota', 'Mississippi',
       'Missouri', 'Pennsylvania', 'Rhode Island', 'South Carolina',
       'South Dakota', 'Tennessee', 'Texas', 'Utah', 'Vermont', 'Virginia',
       'Washington', 'West Virginia', 'Wisconsin', 'Wyoming', 'AL', 'AK', 'AZ',
       'AR', 'CA', 'CO', 'CT', 'DE', 'DC', 'FL', 'GA', 'HI', 'ID', 'IL', 'IN',
       'IA', 'KS', 'KY', 'LA', 'ME', 'MT', 'NE', 'NV', 'NH', 'NJ', 'NM', 'NY',
       'NC', 'ND', 'OH', 'OK', 'OR', 'MD', 'MA', 'MI', 'MN', 'MS', 'MO',
       'PA', 'RI', 'SC', 'SD', 'TN', 'TX', 'UT', 'VT', 'VA', 'WA', 'WV', 'WI',
       'WY', 'U.S.A.', 'usa', 'Calif', 'St. Louis', 'Pennsylvani', 'Hopkins',
       'Los Angeles', 'Atlanta', 'Charlottesville', 'Columbia University',
       'Rochester', 'Northwell', 'U.S.A', 'ASPB', 'Tucson', 'Duke University',
       'Los Angles', '06520', 'Thomas Jefferson University', 'D.C', 'D.C.',
       'uchicago', 'dartmouth', 'vanderbilt', 'Irvine', 'Gainesville',
       'harvard', 'rush', 'National Children', '10065',
       'Institute for Pure and Applied Knowledge', 'childrenscolorado', 'Colo',
       'UNITED STATES', 'Minneapolis', 'San Francisco', 'United States.',
       'Dallas', 'United States of America.',
       'United States;', 'med.cornell.edu', 'Yale', '60612', '15213',
       'Feinberg', 'Fort Collins', 'Emory',
       'eattle Cancer Care Alliance', 'Colo.', 'Cleveland', 'Omaha', '38105',
       '15236', '20814', 'usuhs.edu',
       'Seattle', 'Icahn School of Medicine at Mount Sinai', 'Brigham',
       'George Mason University', 'arizona.edu',
       'Mount Sinai', 'Icahn', 'Rush University',
       'Division of Palliative Medicine', 'National Institutes of Health',
       '48202', '81432', 'IUPUI', 'ealthPartners Institute',
       'and Spaulding Rehabilitation Hospital',
       'gertz.morie@mayo.edu', '46202-5114;', 'Houston', 'kkz@uci.edu',
       'Loyola University', '10467', '45242',
       'Hennepin Healthcare', 'Brown University',
       'Warren Alpert Medical School', '10003', 'Weill Cornell',
       'matoa@mskcc.org', 'Fred Hutchinson', 'Chapel Hill', 'CDC', '70803',
       '11030', '10016', 'Charleston',
       'Chicago', 'northwestern', 'Mercy Bioethics Center', 'stanford.edu',
       'Memorial Sloan Kettering Cancer Center',
       'Philadelphia', 'Spring Harbor', 'leveland Clinic', '60153', '48108',
       '76508', '08103', 'Mich', 'Mo',
       'Seton Hall', 'lvogen Inc', 'Albany', 'Conn', 'San Diego', '26339',
       '06510', '19611', 'Cincinnati',
       'Aurora', 'Huck Institutes of the Life Sciences', 'Louis University',
       'Spaulding Rehabilitation Hospital',
       'UCSF', '3154645540', 'Minn', 'Rutgers', 'Wash', 'oston University',
       'anderbilt University Medical Center',
       'esleyan University', 'ulane University School of Social Work',
       'Leonard Davis Institute of Health Economics',
       'niversity Spine Center', '02115', 'arvard Medical School', 'Columbus',
       'eorgetown University', 'Lexington',
       'Jackson Laboratory for Genomics Medicine', 'Baltimore', '94143',
       'Ann Arbor', '80045', '20850', '20871']
Grenada = ['Grenada']
Uzbekistan = ['Uzbekistan']
Venezuela = ['Venezuela']
Vietnam = ['Vietnam']
Zambia = ['Zambia']
Zimbabwe = ['Zimbabwe', 'Harare']
Dominica = ['Dominica']
Belize = ['Belize']
Libya = ['Libya']
West_Bank = ['West Bank and Gaza', 'West Bank', 'Gaza']
Guinea_Bissau = ['Guinea-Bissau']
Greenland = ['Greenland']
Mali = ['Mali']
Saint_Kitts = ['Saint Kitts and Nevis', 'Saint Kitts', 'Nevis']
Kosovo = ['Kosovo']
Burma = ['Burma']
MS_Zaandam = ['MS Zaandam']
Botswana = ['Botswana']
Burundi = ['Burundi']
Sierra_Leone = ['Sierra Leone']
Malawi = ['Malawi']
Western_Sahara = ['Western Sahara']
Sao_Tome = ['Sao Tome and Principe', 'Sao Tome', 'Principe']
Yemen = ['Yemen']
Puerto_Rico = ['Puerto Rico']
Czech = ['Czech Republic', 'Czechia', 'CZ']
West_Indies = ['West Indies']
Unknown = ['International Journal of Biological Sciences', 'yoshiharu333',
           'Occupational and Environmental Medicine', 'JBJS',
           'Bauchner is Editor in Chief and Dr Fontanarosa is Executive'
           ' Editor',
           'journalist specialising in psychology and healthcare',
           'nrgasthep@springernature.com',
           'hemantkrsri@gmail.com', 'maja.baretic@kbc-zagreb.hr',
           'zjgong@163.com', 'pranavish2512',
           '200032', 'bjorklund@conem.org', 'souhail.zayet@gmail.com',
           'listed at the end of the article',
           'acknowledged at the end of the article', '(E.J.R.V.B.)', 'S.D.H.)',
           'T.S.)', 'J. X.)', 'epartment of Radiology', 'N.S)',
           'chengxia1568@126.com', '1134', 'EBS Open Bio',
           'F.N.I.H.)', 'J.L.)', 'zjgong@163.com', 'H.S.)', '3500', 'S.C.B.)',
           'H.S)', '6720', '100069', 'edical AI',
           'Department of Obstetrics & Gynecology', '403',
           ' signorellicarlo2307@gmail.com',
           'ational Institute of Oncology Department of Molecular Pathology',
           'C.P.Y.)',
           'ealth Preparedness Partners', 'Department of Woman', 'ON',
           'iamsiri@yahoo.com',
           'Institute of Infectious Disease', 'ssyhxz@126.com', '(ESPRM)',
           'contributed equally to this work', ')', 'N',
           'Research Institute for Frontier Medicine',
           'Systems Immunology Division',
           'National Institute of Infectious Disease',
           'contributed equally to the manuscript', 'equal contribution',
           'authors', 'hpavlidou@gmail.com',
           'regon Health & Science University', 'XXXX', 'JP',
           'Division of Cardiology', 'HK', 'enningbay@hotmail.com',
           '6725', 'EU', '610000', 'epartment of Virology III',
           'Phone: +86 576 85120120', '117594',
           'Faculty of Medicine', '7103', 'raju.mukherjee@iisertirupati.ac.in',
           'Diseases Department',
           'Joint Research Centre', 'Department of Medicine',
           'dpanzirer@helmsleytrust.org', 'ejabbour@mdanderson.org',
           '200020', 'Professor of Clinical Immunohaematology',
           'cjr.wangwei@vip.163.com', 'zhangxw956658@163.com',
           'South Asia', 'Tex', 'Rheumatology and Clinical Immunology',
           'cardiolab@live.com', '01009', '1083',
           'Departments of Radiology', 'Department of Surgery',
           'epartment of Pharmacy',
           'Mental Health Research and Treatment', 'Allergy and Immunology',
           'pfearon@stpatsmail.com',
           'School of Public Health', 'Metabolism and System Research',
           '6722', 'Que', 'Laboratory Medicine',
           'Professor of Child and Adolescent Psychiatry', '550002',
           'lshl97@163.com',
           'epartment of Clinical Psychology', 'Department of Pathology',
           'Respiratory and Critical Care Medicine',
           'and Research Center', 'Educational and Psychological Studies',
           'm.bakouri@mu.edu.sa',
           'Pediatric Intensive Care Unit', 'Inc', 'is editor of this column',
           'Pulmonary and Critical Care Medicine',
           'epartment of Sociology',
           'Department of Bacteriology and Biocontamination Control',
           'ivision of Infectious Diseases',
           'epartment of Anesthesiology', 'wufengyao@126.com',
           'robert.dietz@cuanschutz.edu',
           'niversity of Development Alternative',
           'Institutes of Biomedical Sciences', 'SA',
           'nternal Medicine Dept', 'Hematology Section', 'Rion']

country_dict = {
    'Afghanistan': Afghanistan,
    'Albania': Albania,
    'Algeria': Algeria,
    'Andorra': Andorra,
    'Angola': Angola,
    'Antigua': Antigua,
    'Argentina': Argentina,
    'Armenia': Armenia,
    'Australia': Australia,
    'Austria': Austria,
    'Azerbaijan': Azerbaijan,
    'Bahamas': Bahamas,
    'Bahrain': Bahrain,
    'Bangladesh': Bangladesh,
    'Barbados': Barbados,
    'Belarus': Belarus,
    'Belgium': Belgium,
    'Belize': Belize,
    'Benin': Benin,
    'Bhutan': Bhutan,
    'Bolivia': Bolivia,
    'Bosnia': Bosnia,
    'Brazil': Brazil,
    'Brunei': Brunei,
    'Bulgaria': Bulgaria,
    'Burma': Burma,
    'Burkina Faso': Burkina_Faso,
    'Cabo Verde': Cabo_Verde,
    'Canada': Canada,
    'Cambodia': Cambodia,
    'Cameroon': Cameroon,
    'Central African Republic': Central_Africa,
    'Chad': Chad,
    'Chile': Chile,
    'China': China,
    'Colombia': Colombia,
    'Congo': Congo,
    'Costa Rica': Costa_Rica,
    'Cote d Ivoire': Cote_d_Ivoire,
    'Croatia': Croatia,
    'Cuba': Cuba,
    'Cyprus': Cyprus,
    'Denmark': Denmark,
    'Djibouti': Djibouti,
    'Dominica': Dominica,
    'Dominican Republic': Dominican_Republic,
    'Ecuador': Ecuador,
    'Egypt': Egypt,
    'El Salvador': El_Salvador,
    'Equatorial Guinea': Equatorial_Guinea,
    'Eritrea': Eritrea,
    'Estonia': Estonia,
    'Eswatini': Eswatini,
    'Ethiopia': Ethiopia,
    'Fiji': Fiji,
    'Finland': Finland,
    'Faroe Island': Faroe,
    'France': France,
    'Gabon': Gabon,
    'Grenada': Grenada,
    'Gambia': Gambia,
    'Georgia': Georgia,
    'Germany': Germany,
    'Ghana': Ghana,
    'Greece': Greece,
    'Guatemala': Guatemala,
    'Guinea Bissau': Guinea_Bissau,
    'Guinea': Guinea,
    'Guyana': Guyana,
    'Greenland': Greenland,
    'Haiti': Haiti,
    'Honduras': Honduras,
    'Hungary': Hungary,
    'Iceland': Iceland,
    'India': India,
    'Indonesia': Indonesia,
    'Iran': Iran,
    'Iraq': Iraq,
    'Ireland': Ireland,
    'Israel': Israel,
    'Italy': Italy,
    'Jamaica': Jamaica,
    'Japan': Japan,
    'Jordan': Jordan,
    'Tunisia': Tunisia,
    'Kazakhstan': Kazakhstan,
    'Kenya': Kenya,
    'South Korea': South_Korea,
    'Kosovo': Kosovo,
    'Kuwait': Kuwait,
    'Kyrgyzstan': Kyrgyzstan,
    'Latvia': Latvia,
    'Lebanon': Lebanon,
    'Liberia': Liberia,
    'Libya': Libya,
    'Liechtenstein': Liechtenstein,
    'Lithuania': Lithuania,
    'Luxembourg': Luxembourg,
    'Madagascar': Madagascar,
    'Malaysia': Malaysia,
    'Maldives': Maldives,
    'Mali': Mali,
    'Malta': Malta,
    'Mauritania': Mauritania,
    'Mauritius': Mauritius,
    'Mexico': Mexico,
    'Moldova': Moldova,
    'Monaco': Monaco,
    'Mongolia': Mongolia,
    'Montenegro': Montenegro,
    'Morocco': Morocco,
    'Namibia': Namibia,
    'Nepal': Nepal,
    'Netherlands': Netherlands,
    'New Zealand': New_Zealand,
    'Panama': Panama,
    'Nicaragua': Nicaragua,
    'Niger': Niger,
    'Nigeria': Nigeria,
    'Macedonia': Macedonia,
    'Norway': Norway,
    'Oman': Oman,
    'Pakistan': Pakistan,
    'Palestine': Palestine,
    'Papua New Guinea': Papua_New_Guinea,
    'Paraguay': Paraguay,
    'Peru': Peru,
    'Taiwan': Taiwan,
    'Philippines': Philippines,
    'Poland': Poland,
    'Portugal': Portugal,
    'Qatar': Qatar,
    'Romania': Romania,
    'Russia': Russia,
    'Rwanda': Rwanda,
    'Scotland': Scotland,
    'Saint Lucia': Saint_Lucia,
    'Saint Kitts and Nevis': Saint_Kitts,
    'Saint Vincent': Saint_Vincent,
    'Samoa': Samoa,
    'San Marino': San_Marino,
    'Saudi Arabia': Saudi_Arabia,
    'Senegal': Senegal,
    'Serbia': Serbia,
    'Seychelles': Seychelles,
    'Singapore': Singapore,
    'Slovakia': Slovakia,
    'Slovenia': Slovenia,
    'Somalia': Somalia,
    'South Africa': South_Africa,
    'Spain': Spain,
    'Sri Lanka': Sri_Lanka,
    'Sudan': Sudan,
    'Suriname': Suriname,
    'Sweden': Sweden,
    'Switzerland': Switzerland,
    'Tanzania': Tanzania,
    'Thailand': Thailand,
    'Togo': Togo,
    'Trinidad': Trinidad,
    'Turkey': Turkey,
    'Uganda': Uganda,
    'Ukraine': Ukraine,
    'Uzbekistan': Uzbekistan,
    'United Arab Emirates': United_Arab_Emirates,
    'UK': UK,
    'Uruguay': Uruguay,
    'USA': USA,
    'Vietnam': Vietnam,
    'West Bank': West_Bank,
    'Zambia': Zambia,
    'Zimbabwe': Zimbabwe,
    'Venezuela': Venezuela,
    'MS Zaandam': MS_Zaandam,
    'Botswana': Botswana,
    'Burundi': Burundi,
    'Sierra Leone': Sierra_Leone,
    'Malawi': Malawi,
    'Western Sahara': Western_Sahara,
    'Sao Tome and Principe': Sao_Tome,
    'Yemen': Yemen,
    'Puerto Rico': Puerto_Rico,
    'Czech Republic': Czech,
    'West Indies': West_Indies,
    'Unknown': Unknown
}